...
final class RuleGuidanceCatalog {
    ...
    static RuleGuidance guidance(Finding finding, String localizedTitle, String localizedWhy, String localizedFix, ReportLanguage language) {
        ...
        return switch (code) {
            ...
            case "SPR_ALT021" -> advisor(language, "controllo manuale Principal != null", "@PreAuthorize / SecurityFilterChain / @AuthenticationPrincipal", "https://docs.spring.io/spring-security/reference/servlet/authorization/method-security.html", "if (principal != null) { ... }", "@PreAuthorize(\"isAuthenticated()\")\npublic Result useCase(@AuthenticationPrincipal User user) { ... }");
            case "SPR_ALT022" -> advisor(language, "SecurityContextHolder nel codice business", "security adapter + identity esplicita nel service", "https://docs.spring.io/spring-security/reference/servlet/authentication/architecture.html", "SecurityContextHolder.getContext().getAuthentication()", "public Result useCase(CurrentUser user) { ... }");
            case "SPR_ALT023" -> advisor(language, "controllo ruoli manuale con stringhe ROLE_*", "@PreAuthorize / AuthorizationManager / servizio permessi tipizzato", "https://docs.spring.io/spring-security/reference/servlet/authorization/method-security.html", "if (roles.contains(\"ROLE_ADMIN\")) { ... }", "@PreAuthorize(\"hasRole('ADMIN')\")\npublic void adminUseCase() { ... }");
